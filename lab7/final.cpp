...
public:
    ...
    void loadFreightCars() {
       ...
       // cout << "Vertex Name : " << this -> name << endl;
        for(auto &it : parcel_destination_heaps){
        ...
        //cout << this -> name << " " <<  freight_cars.size() << endl;
...
public:
    ...
    void groupFreightCars() {
        ...
        //cout << "in group Freight Cars" << endl;
...
public:
   ...
   std::map<std::string, std::string> get_state() {
    ...
    while(time_tick <= run_till_time_tick){
        ...
        old_state = get_state();
        //cout << "OLD STATE" << old_state.size() << endl;
    ...
    //cout << "TIME TICK IS : ----------------------------------- " << time_tick << endl;
     ...
     if(result == true)
     {
        if(time_tick == run_till_time_tick)
         this -> graph.moveTrains();
...
//     if (prc.all_parcels_delivered() == false){
//         std::vector<std::string> delivered_parcels = prc.get_delivered_parcels();

//         if (std::find(delivered_parcels.begin(), delivered_parcels.end(), "P31") == delivered_parcels.end()){
//             if (prc.get_parcels_delivered_up_to_time_tick(4).size() == 25){
//                 resultFile << "Test Case 2: Passed\n";
//             }
//             else{
//                 resultFile << "Test Case 2: Failed\n";
//             }
//         }
//         else{
//             resultFile << "Test Case 2: Failed\n";

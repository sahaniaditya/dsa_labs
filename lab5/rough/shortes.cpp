...
Path* PathFinder::findPath(std::string origin, std::string destination) {
    ...
    // std::cout << stops.size() << endl;
    // std::cout << stops[0] -> getLine() -> getLineName();
    // std::cout << " " << stops[1] -> getLine() -> getLineName() << endl;
    for(int i  = 0;i<stops.size();i++){
    ...
    return nullptr;
        ...
        for(int i= 0;i<paths.size();i++){
            vector<MetroStop*> path = paths[i] -> getStops();
            m = (path.size() < m) ? path.size() : m;
        }

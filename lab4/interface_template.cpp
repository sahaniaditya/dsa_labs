...
private:
    ...
    Trip* tripnodeptr;
public:
    BinaryTreeNode(int departuretime, Trip* tripenodeptr, BinaryTreeNode* parentptr)
         {
            this -> tripnodeptr = tripenodeptr;
...
BinaryTreeNode* findSuccessor(BinaryTreeNode* root, int target) {
    ...
    while (root != nullptr) {
        if (root -> getDepartureTime() == target) {
            // If the target node has a right subtree, the successor will be the minimum
            ...
            // If the current node's value is less than the target, move to the right subtree.
            root = root -> getRightPtr();
        } else {
            // If the current node's value is greater than the target, update the successor
...
BinaryTreeNode* findPredecessor(BinaryTreeNode* root, int target) {
    ...
    while (root != nullptr) {
        if (root->getDepartureTime() == target) {
            // If the target node has a left subtree, the predecessor will be the maximum
...
public:
    ...
    std::vector<Trip*> show_trips(std::string pick_up_location, int after_time, int before_time) {
     ...
     std::vector<Trip*> showTripsbydestination(std::string pick_up_location, std::string destination,int after_time, int before_time) {	
        ...
        inorder(node,ans,after_time,before_time);
